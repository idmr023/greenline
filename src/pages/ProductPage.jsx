...
import { ChevronLeft, ChevronRight, Play, Tag, ShieldCheck } from 'lucide-react';
import ProductImage from '../components/ProductImage';
import ChargingCostBadge from '../components/ChargingCostBadge';
...
const FICHA_LABELS = {
  ...
  condiciones_autonomia: 'Condiciones de autonomía',
  capacidad_escalada_pct: 'Capacidad escalada',
...
function formatFichaValue(key, value) {
  ...
  if (key === 'capacidad_escalada_pct') return `${value}%`;
  if (key === 'accesorios' && Array.isArray(value)) return value.join(', ');
  return String(value);
}
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[3fr_2fr] gap-8 mb-10">
        ...
        <div className="flex flex-col">
          ...
          {/* Badge de carga */}
          <ChargingCostBadge cost={chargingCost} />

          {/* GreenLine badge */}
          <div className="flex items-center gap-2 bg-brand/5 border border-brand/20 rounded-lg px-4 py-3 mt-4">
            <ShieldCheck className="w-5 h-5 text-brand shrink-0" />
            <span className="text-sm font-semibold text-brand-dark">
              Incluye Casco + 2 Revisiones Técnicas
            </span>
          </div>
          ...
          <div className="mt-auto pt-6">
            <button
              type="button"
              disabled={product.disponibilidad === 'Fuera de stock'}
              className={`w-full py-3.5 rounded-lg font-semibold text-base transition-colors ${
                product.disponibilidad === 'Fuera de stock'
        ...
        <div className="min-h-[200px]">
          ...
          {activeTab === 'manuales' && (
            <p className="text-gray-500">
              Manuales y guías de usuario disponibles próximamente.
            </p>
          )}
