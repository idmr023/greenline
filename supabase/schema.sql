...
-- Vista maestra usada por el storefront público: productos con
-- categoría, imágenes (ordenadas), colores, ficha técnica e info
-- adicional ya resueltos en el servidor (vista_productos_web).
CREATE OR REPLACE VIEW vista_productos_web WITH (security_invoker = true) AS
SELECT
  p.id,
  p.nombre,
  p.slug,
